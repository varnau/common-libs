...
void graph_plot(char* filename, enum Plot_Type plot_type, graph_t* graph_p)
{
    ...
    {
        k = kh_get(ii,histo,v[i]);
        if(kh_end(histo) == k){
            k = kh_put(ii, histo, v[i] ,&ret);
            kh_val(histo, k) = 0;
        }
        kh_val(histo, k)++;
    }
...
void graph_run_grade_stats(graph_t *graph_p)
{
    ...
    for(int i = 0; i < graph_p->num_vertices; i++)
        graph_p->stats.grade[i] = graph_vertex_grade_i(i, GRAPH_EDGE_OUT, graph_p);
...
void graph_run_path_stats(graph_t *graph_p)
{
    ...
    int *vec        = calloc(graph_p->num_vertices,sizeof(int));
    int *bt         = calloc(graph_p->num_vertices,sizeof(int));
    ...
    for(int i = 0; i < graph_p->num_vertices; i++)
    {
        ...
        for(int j = v_it = 0; j < graph_p->num_vertices; v_it = ++j){
            ...
            max_jumps[v->id] = max_jumps_path;
            max_w[v->id] = max_w_path;
        ...
        vec[i] = graph_vertex_grade_i(i, GRAPH_EDGE_OUT, graph_p);
    ...
    graph_p->stats.grade = vec; 
    graph_p->stats.betweenness = bt;
    graph_p->stats.max_jumps = max_jumps ;
    graph_p->stats.max_w     = max_w     ;
    graph_p->stats.max_jumps_graph = max_jumps_graph;
    graph_p->stats.max_w_graph     = max_w_graph;
